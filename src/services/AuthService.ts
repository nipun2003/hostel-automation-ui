...
class AuthService {
	...
	async getStudentWithReg(regNo: string): Promise<Student> {
	...
	async loginWithRegAndPassword({
		...
		await this.account.createEmailSession(student.email, password);
	...
	async checkRegistrationNumberForNewUser(registrationNumber: string) {
		try {
			...
			if (res.success) {
				throw new Error(
					"Sorry, this registration number is already registered. Please login with your password."
				);
			}
			return res.data as Student;
		} catch (e: unknown) {
			if (e instanceof Error) {
				...
				if (message.includes("ID could not be found")) {
					throw new Error(
						"Sorry, we couldn't find any student with this registration number. Please check and try again."
					);
				}
				throw e;
			}
